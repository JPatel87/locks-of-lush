...
This project is a ficticious hair salon site targetting female clients, typically between the ages of 20-45. Clients are able to view salon services and stylist details from the site and and make a booking with a stylist.
...
Note: All user stories besides the "could have" user stories have been completed in this project. An effort was made to start the stylists CRUD, however, only the stylist model and admin view was initiated. 
...
* As a site user I want to be able to edit my bookings from my account so that I can make changes at a time that is convenient for me.
...
* As a site user I want to be able to add a review after my appointment so that the salon can improve or maintain its services
...
* As a site admin I want to be able to approve reviews before they can be displayed on the website so that I have control over what is being posted on the site.
...
* Navigation links
    ...
    * When the user is logged IN, two additional navbar items are added to the navigation menu. There is the name of the user displayed as "Logged in as [username]". This is displayed in bold. The purpose of this is to portray to and remind the user that they are successfully logged in. There is a dropdown arrow next to the logged in status which holds a link to the logout page. The other extra navigation item is "Bookings" from where users can view/make/change/cancel appointments from.
...
| Image | Description |
| --- | --- |
| ![Navigation](readme_documents/features/navigation.png) | Navigation view when current page is the login page (user not logged on) |
| ![Navigation-hover](readme_documents/features/navigation-hover.png) | Navigation view where the current page is the login page and the services link is hovered over (user not logged on) |
...
* Contact details
    * The contact details consisting of address, telephone number and opening times for the salon is kept in the footer for users to have at hand.
    * A "book now" action button is present to encourage users to book. The button is a different colour to other buttons across the site to highlight its importance. The button also changes colour upon hover for added effect. 
...
| Image | Description |
| --- | --- |
| ![Footer-contact us](readme_documents/features/footer-contact.png) | Footer - contact details |
| ![Footer-contact us hover](readme_documents/features/footer-contact-hover.png) | Footer - contact details with book now button hovered over
| ![Footer-social hover](readme_documents/features/footer-hover.png) | Footer - social media/email links, with twitter link hovered over


#### Page summary
* Heading 
    * Each page has a summary which consists of a heading which serves as a reminder to the users of the subject of the page.
    * All headings are on a grey background and the text is of a different font to the body to emphasize the purpose of the page.
* Body
    * The background colour of the body is coloured in a light pink colour and the text is grey to make it stand out on the background. The text in each body is kept to a minimum with only key information to keep the reader interested.

| Image | Description |
| --- | --- |
| ![Services page summary](readme_documents/features/page-container.png) | Services page summary |

#### Buttons (besides footer "book now" button)
...
The main goals of the home page are to attract the users attention and provide a brief summary of what the salon offers.

* Hero image 
    * The hero image selected is striking and easily conveys the purpose of the site (ie. hair services) for a first time user and inspires them to visit the rest of the site content. 
...
* Services summary 
    ...
    * If an admin user is logged on - an "Add service" button link is also present in the summary. Regular users will not see this button. This is useful for hairdresser businesses who regularly update their services. 

* Add service
    * By clicking on the "Add service" button the admin user will be directed to the "Add service" page.
    * A form will display with the following fields; Service name, Service type (dropdown list) and price. 
    * The service name will automatically capitalise upon successful submission of a form, so if the admin user has entered this in lowercase, it will not matter. This helps keep the formatting of the services consistent and allows for better user experience.
    * The service name is required to be unique - so if the form is submitted with an iteration of an existing service name, an error message will appear informing the user that a service with the same name already exists - this is to prevent any confusion to users if they see two services with the same name. 
    * All fields are required - so if any information is left out the form will not submit and the admin user will be notified to either fill in the field or select an item.
    ...
    * An error message will be displayed if the admin user inputs a price below £15, above £300 or greater than 3 digits before the decimal place (e.g £1000). This is because upon my research of hairdresser prices, it is unlikely that a service is going to be less than £15 or above £300 therefore if this is the case, it is likely that the admin had made a mistake. 

* Services accordion
    * The services accordion was created using the accordion flush template supplied by [Bootstrap](https://getbootstrap.com/docs/5.0/components/accordion/).
    * The services accordion consists of three accordion items each holding information about three separate types of services.
    * The services in each accordion item are ordered in alphabetical order to ensure a consistent layout across all three accordion items.
    * The reason an accordion was selected as a means to convey service details is because it keeps the site looking minimalistic and elegant and adds a level of user interactivity.
    * Each accordian item contains service names and prices.
    * If an admin user is logged on, then below each service price, the admin will see links to "edit" or "delete" services. Regular users will not see these options.
...
* Edit service  
    ...
    * At the end of the form the admin user is given the option to confirm or cancel the edit.These options are useful as they serve as an additional safety measure to prevent the user from making a change they do not intend to.  
...
* Delete service
    ...
    * If they confirm - the user will be re-directed back to the bookings page, a success message will be displayed at the top of the services page to say that the request was successful and the service will no longer appear in the service accordion table.
...
| ![Add service incorrect form, price low](readme_documents/features/add-service-price-low.png)| Add service page - form errors, price too low |
| ![Add service incorrect form, price high](readme_documents/features/add-service-price-high.png)| Add service page - form errors, price high |
| ![Add service incorrect form, price too high](readme_documents/features/add-service-price-too-high.png)| Add service page - form errors, price too high |
| ![Services accordion](readme_documents/features/services-accordion.png) | Services accordion |
| ![Style accordion item](readme_documents/features/style-accordion-item.png) | Services accordion - style acordion item selected (non-admin view)|
...
* Stylist details
    * Each salon stylist has their name, image and brief experience details captured on the page so that users are able to relate to the stylists in order to make an informed decision when it comes to booking.
...
* Login, sign up and log out requests are handled by [Django allauth](https://django-allauth.readthedocs.io/en/latest/installation.html), only the layout of the forms was manipulated by the project owner.
* All pages have been designed in the same manner as the page summaries so that the user is able to easily develop familiarity with the site layout. 
* When users log on they are directed to the home page.
* The logout page has a button to cancel the request - in case the user had made a mistake to select logout. If the cancel button is selected the user is re-directed back to the bookings page. If the logout button is selected the user is logged out and redirected back to the home page.  
...
| Image | Description |
| --- | --- |
| ![Login](readme_documents/features/login-error.png) | Login page with allauth error message |
...
* Make booking
    * There are two separate "make booking" forms - one for the admin and one for the user. The only difference between them is that the admin form displays the user field to enable the admin to select a user from a dropdown list to book an appointment for. Whereas the user form does not display the user id field - it takes the logged in user's username automatically from the booking they make. This is to stop users from having control over any other users bookings.
    * The "Make booking" form contains the following required fields; client first name, client last name, date, time, stylist and service.
...
* View bookings
    * The table of bookings is a responsive table which means on small screens such as mobile phone devices, the table itself can be scrolled across rather than the page being scrolled, which calls for better user experience.
    ...
    * If the "Change" button is selected then the change booking form is called.
    * If the "Cancel" button is selected then the cancel bookings form is called.
...
* Cancel bookings
    * The "Cancel booking" page displays a message to the user relaying the appointment details (service, date, time and stylist) they requested to cancel and asks them to confirm or cancel the deletion.
    * If they confirm - the user will be re-directed back to the bookings page, a success message will be displayed at the top of the bookings page to say that the request was successful and the booking will no longer appear in the bookings table.
...
| ![View bookings](readme_documents/features/bookings-view.png) | Bookings table, showing appointments in the past that cannot be changed or cancelled and appointments in the future that can be changed or cancelled.
| ![Cancel bookings](readme_documents/features/bookings-delete.png) | Cancel booking page example |
...
* Home - contains only the home page
* Services - contains service model, service home page and add/edit/delete services pages
* Stylists - contains stylist model and stylist home page
* Bookings - contains booking model, booking home page and add/edit/delete booking pages.
...
Note: White has been used as the body background colour. 

| Image | Description |
| --- | --- |
| ![colour-scheme](readme_documents/colour_scheme/colour-palette.png) | Site colour scheme |
...
| --- | --- | --- | --- |
| Site has home page with a logo, hero image and catchy slogan | Yes | [Home page](#home-page), [Imagery](#imagery)  | 1. Specify sizes for images on the home page, as lighthouse testing showed reduced site performance due to sizes not being sepcified for these images 2. Use a larger version of the favicon icon as part of the site logo in the navigation to improve the visual appearance of the site. |
...
| Services page can be accessed from the home page | Yes| [Navigation](#navigation), services page appearS as a navigation link, so can be accessed from any page when the user is logged on or not | None |
...
| Stylist page includes stylists name, image and brief background details. | Yes | [Stylists](#stylists-page) | Make the image field a non-required field and have a salon placeholder image if stylists do not have/want to provide an image for the site. |
...
| Acceptance criteria | Achieved | Evidence | Future improvements |
| --- | --- | --- | --- |
|The user account is set up so users can view all of their past and upcoming bookings, once they log into their account.|Yes | [Bookings](#bookings-page), there is a "View bookings button" in the bookings page which opens up the bookings table | Separate out past and upcoming bookings into separate collapsible tables with separate button links, to improve user experience so that users dont have to trail through lists of past bookings before finding an upcoming booking. |
...
| Changes made to the service name and price are reflected on the site | Yes | Manual check, any changes made to the service is reflected back on the site for logged in and non-authenticated users to see | None |
...
To determine how much of the code was tested by automated testing, a django tool called [Django coverage](https://pypi.org/project/django-coverage/) was installed, this revealed that 98% of the services app code had been tested, however there was still some missing tests in the views.py file. In order to determine what code was still left to test, coverage enables an interactive HTML report to be created. Upon reviewing this report - it revealed that the error messages in the add and edit services page had not been tested. The tests for these items was covered by manual testing. The results of coverage can be seen from the link below. 
...
* Service accordion items - on smaller screens stack vertically, then as the screen size gets bigger they appear as two on one row and the third on the row below.
* Stylists images - on smaller screens stack vertically, then as the screen size gets bigger they appear as two on one row and the two on the row below.
* Bookings table - on smaller screens can be scrolled horizontally

[Home page responsive view](readme_documents/validation/home-responsive.png) 

[Services page responsive view](readme_documents/validation/services-responsive.png) 

[Stylists page responsive view](readme_documents/validation/stylists-responsive.png) 

[Login responsive view](readme_documents/validation/login-responsive.png) 
...
* [Lighthouse test - change booking page](readme_documents/validation/lighthouse-change-booking.png)
...
* [Techshinobi.com](https://tekshinobi.com/setting-active-navbar-link-in-django-template/) - how to create navbar active links in django
